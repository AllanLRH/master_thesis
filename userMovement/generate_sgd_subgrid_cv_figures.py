...
description_file = "figs/crossval_subgrid_pca_alpha_descriptions.json"
description_dict = dict()

scores = cvr[['mean_train_score', 'mean_test_score', 'std_train_score', 'std_test_score']]
...
description_dict = scores.describe().to_latex()

to_plot_mean = cvr.groupby(['param_pca__n_components', 'param_sgd__alpha']).mean_test_score.mean().T.unstack()
to_plot_std  = cvr.groupby(['param_pca__n_components', 'param_sgd__alpha']).std_test_score.mean().T.unstack()
...
with sns.axes_style('ticks'):
    ...
    plt.tight_layout()
    fig.savefig(f'figs/crossval_pca_alpha_fine_mean_score.pdf')
with sns.axes_style('ticks'):
    fig, ax = plt.subplots(figsize=[12, 7])
    sns.heatmap(to_plot_mean, ax=ax, cmap='Greys_r')
    ax.set_xlabel(r'$\alpha$')
    ax.set_ylabel('PCA components')
    ax.set_xticklabels(xticklabels, rotation='vertical')
    ax.set_yticklabels(yticklabels)
    plt.tight_layout()
    fig.savefig(f'figs/crossval_pca_alpha_fine_mean_score_nolimits.pdf')
with sns.axes_style('ticks'):
    fig, ax = plt.subplots(figsize=[12, 7])
    ...
    ax.set_yticklabels(yticklabels)
    plt.tight_layout()
    fig.savefig(f'figs/crossval_pca_alpha_fine_std_score.pdf')
