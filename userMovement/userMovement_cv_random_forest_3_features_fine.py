...
jn.send(message="Started fine CV for RF grid with just 3 features: B7, B3 and G7.")
...
try:
    ...
    param_grid = {
        'rf__n_estimators': np.arange(3, 40, 3),
        'rf__max_depth': np.arange(3, 13),
    ...
    fig.savefig('figs/userMovement_cv_random_forrest_roc_curve_3_features_fine.pdf')
    ...
    est.x_va = x_va
    est.y_va = y_va
    with open("userMovement_rf_3_features_fine.pkl", 'bw') as fid:
        pickle.dump(est, fid)
