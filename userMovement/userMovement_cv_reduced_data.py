...
import logging
logging.basicConfig(filename='userMovement_sgd_reduced_data.log', level=logging.INFO,
                    format="%(asctime)s :: %(filename)s:%(lineno)s :: %(funcName)s() ::    %(message)s")
logger = logging.getLogger('userMovement_sgd_reduced')


data_path = '../../allan_data/DataPredictMovement_half.p'
x, y = np.load(data_path)
del x
x_hqm = pd.read_msgpack('../../allan_data/DataPredictMovement_half_quarter_comb_hourly_mean.msgpack').values
x_qm = pd.read_msgpack('../../allan_data/DataPredictMovement_half_quarter_comb_mean.msgpack').values
x_lst = [(x_hqm, 'x_hqm'), (x_qm, 'x_qm')]
...
processes = 24
try:
    for x, xname in x_lst:
        logger.info(f"Processing {xname}")
        ...
        pipe = Pipeline([('scaler', preprocessing.StandardScaler()), ('sgd', SGDClassifier())])
        param_grid = {
        ...
        fig.savefig(f'figs/userMovement_cv_roc_curve_{xname}.pdf')
